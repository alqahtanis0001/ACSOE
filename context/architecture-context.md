...
| Scout tallies | SQLite, table `scout_tallies` (migration 0007, Phase 7, spec 146) | One write-once row per tick on which engine 7 ran, **including ticks with no candidate**: engine 7's published payload stored verbatim (scanned, entered, excluded by reason, the tradable universe, the ranking skip tally, the full ranked list with each expected move, the candidate, the reason code), plus engine 3's `closed_bar_ts`. It is the funnel's universe step (operator ruling Q8, 2026-09-19). A tick-level fact, never a rejection. Excluded pairs are counted by reason, not named; which pair was excluded for which reason is recomputable after a replay from the committed fixtures, checked against these counts |
