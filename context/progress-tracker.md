...
- **An intermittent native memory fault in the seed write path.** Roughly 20% of full-suite runs die rather than reporting a verdict, always inside `seed.py` → `write_trade` / `write_position` → pydantic `model_dump`, with three different Windows fault statuses. **Not root-caused**; pyarrow, `pytest-asyncio`, test ordering, `root_import_path` and the pydantic-core version were each ruled out by test, and the one experiment that would have separated software from silicon was overridden by the power plan. Hardware is suspected and is out of scope. Mitigated by a crash-aware retry in `toolchain_green`: a crash is retried once and named in the PASS, a verdict is never retried, and a second crash FAILs. This is a closed question, not an open one — see `docs/build-log/phase-0.md`. It re-opens if the fault appears on other hardware, appears outside the seed write path, or starts failing the retry.
  ...
  **And then the re-run crashed twice, which is the 4% the register predicts.** `p8-f3c` died with `0xC0000005 ACCESS_VIOLATION` on both the first attempt and the crash-aware retry, three to four minutes into the suite, at **two different tests** — `tests/research/test_training_main.py::test_the_ranking_study_refuses_without_a_dataset` and then `tests/engines/test_prediction.py::test_a_di_fitted_without_the_exclusion_span_blocks_rather_than_loading`. Two different sites is the fault's signature rather than a defect's, and neither test is near the boundary being gated. Machine health was checked rather than assumed: 56 GB of 95.7 GB free, so nothing was actionable. **Three consecutive red gates on one boundary, none of them caused by the content under test after the first** — that is a materially worse run of luck than the register's 4% suggests should be common, and it is recorded here because the honest reading is that this session sampled the fault far more heavily than the estimate, exactly as the Phase 2 close said one session might.

  **`p8-f3d` then crashed twice too: four consecutive crashes on one boundary, p ~ 0.0016 at the recorded 20% rate.** The second attempt died **1-2% into the suite** inside `pydantic/main.py model_dump`, called from `engines/market_sensor/contracts.py:225 to_state` through an orchestrator tick in `research/backtest.py` - **the register's primary site, named outright**. The attempts were 49 s apart, against three to four minutes for the previous pair, so the rate is not stable even within one night. One environmental hypothesis was tested and refused: all four crashes were in the `ACSOE-gate` worktree while the same suite went green in `ACSOE-gate2` and `ACSOE-gate3` within the hour, and `git clean -fd --exclude=logs/` carries no `-x` so the ignored caches survive every gate - but they measure 510 K / 404 K / 18 M there against 454 K / 404 K / 17 M and 366 K / 403 K / 17 M, no anomaly. **Either this session sampled catastrophically badly or the rate has risen, and one session still cannot tell those apart.** The operator should know that the Phase 8 boundary gates cost four full suite runs to obtain one verdict.

  **Then `p8-f3e` crashed twice in a second worktree: six consecutive crashes, eight distinct sites, and the last two in pure Python.** `clients/kraken/replay.py:166 ensure` (from the paper broker's `_observe_trades`) and `research/labelling.py:339 _label_at` - **no C extension in the frame at all**. An `ACCESS_VIOLATION` inside plain Python means the interpreter's own memory is corrupt, and eight sites with no repetition is the opposite of a defect's signature. Ruled out by measurement, not assumption: the worktree (two of them), and resource pressure - 56.4 GB of 95.7 GB RAM free, 65 GB disk free, ten `python.exe` (the recorder family), and **no WHEA, Kernel-Power, disk, NTFS or volmgr error or warning in the Windows System log for six hours**.
