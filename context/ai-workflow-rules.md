...
Every criterion must run offline with no network and no API key. Where a criterion concerns live behaviour, it checks a recorded artefact on disk. A separate `--live` flag enables the network checks and is never required for a phase to be green.
