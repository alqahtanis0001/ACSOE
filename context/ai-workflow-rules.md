...
**Engine 20 `tournament` is a dependency of Phase 6, not only of Phase 7.** Its leaderboard is what engine 14 `adaptive_router` reads in Phase 6, so a model phase that closes without it leaves Phase 6 nothing to weight by. The table above places engine 20 in Phase 7 while engine 14 sits in Phase 6; that ordering is unresolved and is the operator's to settle before Phase 6 is scoped.
