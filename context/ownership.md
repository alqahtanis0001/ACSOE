...
| The ranking feature, `scout.rank_feature` and `scout.rank_descending`. **Ruled 2026-09-19 (R1, spec 75 resolved): `expected_move`**, set per run by the replay driver; absent means alphabetical, which is the Phase 7 baseline. | Lead (config) | B (7 `scout`, `rank_universe`) | `config/default.yaml`, `engines/scout/contracts.py` |
...
| The exit-cycle equity row's inputs: per-trade `net_proceeds` and per-position `value`, and the `cash_source` column. **Phase 6, operator ruling 2026-09-17.** | B (22 `exit`, 21 `position_manager`, migration 0005 — spec 113) | C (19 `memory` — spec 114) | `engines/exit/contracts.py`, `engines/position_manager/contracts.py`, `db/migrations/`, `context/engine-contracts.md` |
...
| **Phase 7 seams, 2026-09-19 (spec 126 step 8).** The shared ranking function: expected move for the filtered universe, skipping incomplete vectors and anomaly or DI refusals (R11). | C (`modelling/`, spec 144) | B (7 `scout`, spec 144) | `src/acsoe/modelling/` |
| The recorded `AssetPairs` (REST, verbatim, plus the websocket v2 `instrument` snapshot for keying by the engines' pair names) | A (127) | A (129 replay client), C (141 candle criterion) | `tests/fixtures/kraken/` |
| The weekly time-and-sales partitions for the window and its warm-up | A (128) | A (129) | `data/derived/trades_weekly/` manifest |
| The declared scenario fixtures: the bucket table (A, 130) and Kraken's fee schedule (committed by the lead 2026-09-19) | A, Lead | A (129 replay client only; nothing else may import them, spec 141) | `tests/fixtures/replay/` |
| The replay scenario's identity, `scenario_digest` and `scenario_description`, exposed on the clients container | A (129, the replay driver builds the container) | Lead (`core/` `_record_run`, 134) | `cli/research.py`, `research/backtest.py` |
| The scenario columns on `runs` | B (migration 0006, 132) | Lead (134), C (138 attribution) | `db/migrations/`, `clients/store/contracts.py` |
| The approval economics: a write-once `approvals` table keyed by the entry order's `userref`, and the four figures plus three model run ids on `trades` (lead decision D6) | B (migration 0006, 132) | C (19 `memory` writes both, 133; 138 reads) | `db/migrations/`, `clients/store/contracts.py` |
| The Phase 7 run directories: each fold's predictor, calibrators, scaler, forest, `di.npz` and the **capped** skeptic, written once and complete | C (135, 136) | A (131 driver, switching per fold), engines 8, 13, 15, B (7 `scout`) | `modelling/artefacts.py` |
| The trial ledger, one row per trial, fixed before the run launches | C (139) | C (20 `tournament`, the promotion gate), Lead (143) | `docs/dataset/phase-7-trial-ledger.json` |
| The orchestrator's `previous_now` seed, replay resume only (lead decision D9) | Lead (`core/`) | A (131 driver) | `core/orchestrator.py`, `context/engine-contracts.md` |
