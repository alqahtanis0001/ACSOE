...
2. In the same module declare `Config`, `Clock` and `Clients` as `typing.Protocol`s.
   `platform/` and `clients/` provide implementations; `core/` never imports them.
3. `EngineResult.data` must be JSON-serialisable; add a validator rejecting numpy arrays,
   dataframes and model objects.
4. `reason` is mandatory when `blocks_trading` is true; enforce it in the model.
5. Add `tests/core/test_contracts.py` covering the JSON-serialisable rule, the mandatory
