...
3. **mypy `python_version = "3.12"`; no numpy pin; no venv change.** `requires-python` stays
   3.11; ruff `target-version = "py311"` guards the floor and is proven to catch a PEP 695
   `type` statement and the Phase 4 f-string defect. A-2 lands it first, with a probe proof and
   a test pinning the ruff target. Until it lands, run
   `mypy --strict --python-version 3.12 src/ scripts/` yourself; the unmodified gate checks
   nothing under `src/`.
